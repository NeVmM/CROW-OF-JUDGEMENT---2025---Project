        ...
        if (!is_attacking && attack_cooldown <= 0)
        {
            ...
            if (arms_on_ground && player_on_ground)
            {
                ...
                else
                {
                    ...
                    sprite_index = Sprite_EnemyLightSlime_Attack;
                    image_index = 0;
                    image_speed = 1.5;
					
					audio_play_sound(LightSlimeAttack, 10, false);
					audio_sound_gain(LightSlimeAttack, 1.0, 0); // Optional: Set volume
...
if (is_attacking)
{
    if (!attack_effect_spawned && image_index >= 1)
    {
        ...
        var effect_y = y;
        var fx = instance_create_layer(effect_x, effect_y, layer, oEnemyLightSlimeAtkFX);
        fx.owner = id;
        fx.facing = facing;
    ...
    {
        is_attacking = false;
        sprite_index = Sprite_EnemyLightSlime_Idle;
...
if (!is_attacking)
{
    if (is_moving)
    {
        sprite_index = Sprite_EnemyLightSlime_Walking;
    }
    else
    {
        sprite_index = Sprite_EnemyLightSlime_Idle;
